...
{
  private readonly IConnectionManager<ITubeFurnaceConnection> _connectionManager;
  ...
  public TubeFurnaceManager(IDeviceCommandInterpreterRepo deviceCommandInterpreterRepo,
    IConnectionManager<ITubeFurnaceConnection> connectionManager,
  ...
  public async Task<ITubeFurnace> Update(TubeFurnaceConfig config)
  {
    ...
    if (existingFurnace.Connection.Name == config.PortName)
      if (existingFurnace.Connection is SimSyringePumpConnection && config.Simulated || existingFurnace.Connection is SyringePumpConnection && !config.Simulated)
