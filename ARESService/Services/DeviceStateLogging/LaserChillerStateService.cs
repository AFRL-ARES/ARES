...
public class LaserChillerStateService : ChillerStateLogging.ChillerStateLoggingBase
{
  ...
  public LaserChillerStateService(IDbContextFactory<AresDbContext> dbContextFactory, DeviceIdHelper deviceIdHelper)
  {
  ...
  public override async Task<DevicesResponse> GetAvailableDevices(Empty request, ServerCallContext context)
  {
    ...
    var deviceIds = await dbContext.DeviceConfigs
      .Where(cfg => cfg.DeviceType == typeof(ILaserChiller).FullName)
      .Select(s => s.UniqueId)
      .ToListAsync();
